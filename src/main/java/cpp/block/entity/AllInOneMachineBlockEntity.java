...
public void tick(World world, BlockPos pos, BlockState state) {
	...
	if (!world.isClient) {
		...
		if (getTemperature() == Degree.HIGH && getPressure() == Degree.HIGH && (recipe = ORE_RECIPES.get(ingredient())) != null) {
		...
		if (recipe != null) {
			...
			if (Utils.canInsert(recipe.maximize(this), this, 3, 5)) {
				if (workTime >= workTimeTotal) {
					workTime = 0;
					expStorage -= recipe.experience;
					for (int i = 1; i < 3; i++) {
						if (!AllInOneMachineRecipe.UNCONSUMABLE.contains(getStack(i).getItem())) {
							getStack(i).decrement(1);
						}
					}
					Utils.insert(recipe.produce(this), this, 3, 5);
				} else {
					workTime++;
				}
			}
...
public boolean canInsert(int slot, ItemStack stack, Direction dir) {
	return super.canInsert(slot, stack, dir)
	  || slot == 1 && stack.getItem() != getStack(2).getItem()
